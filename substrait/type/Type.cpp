...
ParameterizedTypePtr decodeType<TypeKind::kStruct>(
  ...
  if (isParameterized) {
    ...
    for (int i = 0; i < parameterTypes.size(); i++) {
      types.emplace_back(
          std::dynamic_pointer_cast<const Type>(parameterTypes.at(i)));
    }
...
ParameterizedTypePtr decodeType<TypeKind::kDecimal>(
  ...
  if (isParameterized) {
    ...
    if (common::NumberUtils::is_number(precision->value()) &&
        common::NumberUtils::is_number(scale->value())) {
...
ParameterizedTypePtr decodeLenghBaseType(
    bool isParameterized,
    bool nullable,
    const std::vector<ParameterizedTypePtr>& parameterTypes) {
  auto length =
  ...
  if (isParameterized) {
    ...
    if (common::NumberUtils::is_number(length->value())) {
      return std::make_shared<TypeTag>(std::stoi(length->value()), nullable);
    } else {
      SUBSTRAIT_FAIL(
          "Fail decode to {} type, length parameter must be a positive number",
...
ParameterizedTypePtr decodeType<TypeKind::kVarchar>(
    ...
    bool nullable,
    const std::vector<ParameterizedTypePtr>& parameterTypes) {
  return decodeLenghBaseType<ParameterizedVarchar, Varchar, TypeKind::kVarchar>(
      isParameterized, nullable, parameterTypes);
}
